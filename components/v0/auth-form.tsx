...
export function AuthForm({ type }: AuthFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-4">
      <div>
        <Input id="email" name="email" type="email" placeholder="Email" required autoFocus className="w-full" />
      </div>
      <div>
        <Input
          id="password"
      ...
      {state?.type === 'error' && <div className="text-sm text-red-500">{state.message}</div>}
