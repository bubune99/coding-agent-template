...
import { Reasoning, ReasoningTrigger, ReasoningContent } from '@/components/v0/ai-elements/reasoning'
import { Task, TaskTrigger, TaskContent, TaskItem, TaskItemFile } from '@/components/v0/ai-elements/task'
...
export const ThinkingSectionWrapper = ({
  ...
  return (
    ...
    >
      ...
      <ReasoningContent>
        {thought || (typeof children === 'string' ? children : 'No thinking content available')}
      </ReasoningContent>
    </Reasoning>
  )
}
...
export const TaskSectionWrapper = ({
  ...
  return (
    <Task className="w-full mb-4" defaultOpen={!collapsed} onOpenChange={(open) => onCollapse?.()}>
          ...
          parts.map((part, index) => {
            ...
            if (part && typeof part === 'object') {
              ...
              if (partObj.type === 'starting-repo-search' && partObj.query) {
                return <TaskItem key={index}>Searching: "{partObj.query}"</TaskItem>
              }

              if (partObj.type === 'select-files' && Array.isArray(partObj.filePaths)) {
                return (
                  <TaskItem key={index}>
                    Read{' '}
                ...
                return (
                  <TaskItem key={index}>
                    ...
                    {partObj.changedFiles.map((file: any, i: number) => (
                      <TaskItemFile key={i}>{file.fileName || file.baseName}</TaskItemFile>
                    ))}
                  </TaskItem>
                )
              }
              ...
              if (partObj.type === 'got-results' && partObj.count) {
                return <TaskItem key={index}>Found {partObj.count} results</TaskItem>
              }

              if (partObj.type === 'finished-web-search' && partObj.answer) {
                return (
                  <TaskItem key={index}>
                    <div className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed">{partObj.answer}</div>
                  </TaskItem>
                )
              }
              ...
              if (partObj.type === 'design-inspiration-complete' && Array.isArray(partObj.inspirations)) {
                return (
                  <TaskItem key={index}>
              ...
              if (partObj.type === 'thinking' || partObj.type === 'analyzing') {
                return (
                  <TaskItem key={index}>
                    <div className="text-gray-600 dark:text-gray-400 text-sm italic">Thinking...</div>
                  </TaskItem>
                )
              }
              ...
              if (partObj.type === 'processing' || partObj.type === 'working') {
                return (
                  <TaskItem key={index}>
                    <div className="text-gray-600 dark:text-gray-400 text-sm">Processing...</div>
                  </TaskItem>
                )
              }
              ...
              if (message) {
                return (
                  <TaskItem key={index}>
                    <div className="text-gray-700 dark:text-gray-300 text-sm">{message}</div>
                  </TaskItem>
                )
              }
              ...
              if (taskType !== 'unknown') {
                ...
                return (
                  <TaskItem key={index}>
                    <div className="text-gray-600 dark:text-gray-400 text-sm">{readableType}</div>
                  </TaskItem>
                )
              }
...
export const CodeProjectPartWrapper = ({
  ...
  return (
    <div className={`my-6 border border-border dark:border-input rounded-lg ${className || ''}`} {...props}>
      <button
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="font-medium text-gray-900 dark:text-gray-100">{title || 'Code Project'}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-500 dark:text-gray-400 font-mono">v1</span>
          <svg
            ...
            <div className="p-4">
              <div className="space-y-2 mb-4">
                <div className="flex items-center gap-2 text-sm text-black dark:text-white">
                  ...
                  <span className="font-mono">{filename || 'app/page.tsx'}</span>
                </div>
              </div>
            </div>
          )}
...
const CustomTaskSectionWrapper = (props: any) => {
  ...
  if (
    props.parts &&
    props.parts.some((part: any) => part && typeof part === 'object' && part.type === 'code-project')
  ) {
    const codeProjectPart = props.parts.find(
      (part: any) => part && typeof part === 'object' && part.type === 'code-project',
    )
  ...
  if (props.type === 'task-generate-design-inspiration-v1') {
    return <TaskSectionWrapper {...props} title={props.title || 'Generating Design Inspiration'} />
  }
...
export const sharedComponents = {
  ...
  ol: {
    className: 'mb-4 ml-6 list-decimal space-y-1 text-gray-700 dark:text-gray-200',
  },
  li: {
    className: 'text-gray-700 dark:text-gray-200',
  },
  blockquote: {
    className: 'mb-4 border-l-4 border-gray-300 dark:border-gray-600 pl-4 italic text-gray-600 dark:text-gray-400',
  },
  code: {
    className: 'rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 text-sm font-mono text-gray-900 dark:text-gray-100',
  },
  pre: {
    className: 'mb-4 overflow-x-auto rounded-lg bg-gray-100 dark:bg-gray-800 p-4',
  },
  a: {
