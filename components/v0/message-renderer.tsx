...
function preprocessMessageContent(content: MessageBinaryFormat): MessageBinaryFormat {
...
export function MessageRenderer({ content, messageId, role, className }: MessageRendererProps) {
  // If content is a string (user message or fallback), render it as plain text
  if (typeof content === 'string') {
    return (
      <div className={className}>
        <p className="mb-4 text-gray-700 dark:text-gray-200 leading-relaxed">{content}</p>
      </div>
    )
  }
