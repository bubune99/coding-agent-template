...
import { Message, MessageContent } from '@/components/v0/ai-elements/message'
import { Conversation, ConversationContent } from '@/components/v0/ai-elements/conversation'
import { Loader } from '@/components/v0/ai-elements/loader'
...
export function ChatMessages({
  ...
  if (chatHistory.length === 0) {
    return (
      <Conversation>
        <ConversationContent>
          <div>{/* Empty conversation - messages will appear here when they load */}</div>
        </ConversationContent>
      </Conversation>
    )
  }
          ...
          {chatHistory.map((msg, index) => (
            <Message from={msg.type} key={index}>
              {msg.isStreaming && msg.stream ? (
                ...
                <MessageRenderer content={msg.content} role={msg.type} messageId={`msg-${index}`} />
              )}
            </Message>
          ))}
          {isLoading && (
