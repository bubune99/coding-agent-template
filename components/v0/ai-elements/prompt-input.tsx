...
import { ArrowUpIcon, ImageIcon, Loader2Icon, MicIcon, MicOffIcon, SquareIcon, XIcon } from 'lucide-react'
...
export const savePromptToStorage = (message: string, attachments: ImageAttachment[]) => {
  try {
...
export const createImageAttachmentFromStored = (stored: StoredPromptData['attachments'][0]): ImageAttachment => {
...
export const PromptInputMicButton = ({ className, onTranscript, onError, ...props }: PromptInputMicButtonProps) => {
  ...
  return (
    <PromptInputButton
      ...
      {...props}
    >
      {isListening ? <MicOffIcon className="size-4 text-red-600 dark:text-red-400" /> : <MicIcon className="size-4" />}
    </PromptInputButton>
  )
}
...
export const PromptInputImageButton = ({ className, onImageSelect, ...props }: PromptInputImageButtonProps) => {
  ...
  return (
    <>
      ...
      <PromptInputButton className={cn(className)} onClick={handleClick} {...props}>
...
export const PromptInputImagePreview = ({ attachments, onRemove, className }: PromptInputImagePreviewProps) => {
  if (attachments.length === 0) return null

  return (
    <div className={cn('flex flex-wrap gap-2 p-2', className)}>
      {attachments.map((attachment) => (
        <div key={attachment.id} className="relative group rounded-lg overflow-hidden border bg-muted">
