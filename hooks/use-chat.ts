...
export function useChat(chatId: string) {
  ...
  const {
    ...
    onSuccess: (chat) => {
      ...
      if (chat.messages && chatHistory.length === 0 && !(handoff.chatId === chatId && handoff.stream)) {
        setChatHistory(
  ...
  const handleSendMessage = async (e: React.FormEvent<HTMLFormElement>, attachments?: Array<{ url: string }>) => {
    e.preventDefault()
  ...
  const handleStreamingComplete = async (finalContent: any) => {
    ...
    console.log('Stream completed with final content:', JSON.stringify(finalContent, null, 2))
      ...
      const searchForChatId = (obj: any) => {
        if (obj && typeof obj === 'object') {
          ...
          if (!newChatId && obj.id && typeof obj.id === 'string') {
            ...
            if ((obj.id.includes('-') && obj.id.length > 20) || (obj.id.length > 15 && obj.id !== 'hello-world')) {
